...
describe('sumOfTwo', () => {
  it('should return an array with the index of the elements whose value adds up to the specified target.', () => {
    
    const nums = [1, 3, 8, 11, 6, 21]
    expect(sumOfTwo(nums, 19)).toEqual([2, 3])
    expect(sumOfTwo(nums, 32)).toEqual([3, 5])
    expect(sumOfTwo(nums, 33)).toEqual([])
  })
})